...
const int rele1 = 26;
const int rele2 = 27;
const int rele3 = 14;  // estrella
const int rele4 = 12;  // triángulo
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ESP32_MOTOR")) {
      ...
      client.subscribe("esp32/rpm");   // SUSCRIPCIÓN CORRECTA
    } else {
      Serial.print("Error: ");
...
String generarPagina() {
  ...
  h += "<meta http-equiv='refresh' content='1'>";  // AUTO UPDATE
  ...
  h += "<p>Velocidad actual: " + String(velocidad) + " RPM</p>";
